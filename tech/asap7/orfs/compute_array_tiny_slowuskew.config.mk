# compute_array_tiny_slowuskew — A2 experiment archive.
#
# Hypothesis: a reversed useful-skew SDC could compensate for the
# remaining inter-cell hold by telling STA the outward cells have
# NEGATIVE source latency (their clock arrives EARLIER). The prior
# useful-skew attempt had the wrong sign (outward = later), making
# hold worse. This variant inverts the sign via
# compute_array_tiny.useful_skew_rev.sdc.
#
# Result: identical to "slow" — post-route setup 0, hold 0, 0
# violations, fmax 440 MHz. Useful-skew was a no-op because at 2.5 ns
# clock the resizer already closes hold without needing STA-side help.
# Kept as the only on-disk example of a reversed-direction useful-skew
# SDC — if a future config DOES need it (e.g., the full 32x32 at a
# tighter clock), copy the technique.

export PLATFORM     = asap7
export DESIGN_NAME      = compute_array
export DESIGN_NICKNAME = compute_array_tiny_slowuskew

export VERILOG_FILES = /work/build/sv2v/compute_array_tiny_bcast1.v
export SDC_FILE      = /work/tech/asap7/orfs/compute_array_tiny_slowuskew.sdc
