# compute_array_tiny_slowbal — A2 experiment archive.
#
# Hypothesis: above and beyond "slow" (BCAST_PIPE=1 + 2500 ps), force
# OpenROAD parent CTS into a more balanced topology so adjacent cells
# (e.g., cell[1][0] vs cell[2][0]) share a clkbuf sub-tree instead of
# landing in clkbuf_leaf_1 vs clkbuf_leaf_2 (which had 360 ps of inter-
# leaf skew at the 1 GHz target). Tweaks: CTS_CLUSTER_DIAMETER=100,
# CTS_CLUSTER_SIZE=50.
#
# Result: identical to "slow" — post-route setup 0, hold 0, 0
# violations, fmax 440 MHz. At 2.5 ns the cell-to-cell hold was
# already fully repaired by the resizer without needing tighter CTS
# clustering. The knobs are a no-op for this design at this clock.
# Kept for the next time someone is tempted to tune CTS — there's
# nothing to gain on tiny_bcast0 at 2.5 ns.

export PLATFORM     = asap7
export DESIGN_NAME      = compute_array
export DESIGN_NICKNAME = compute_array_tiny_slowbal
...
# A2 Solution 2 additions: try to force OpenROAD parent CTS into a more
# balanced topology so adjacent cells (e.g., cell[1][0]/cell[2][0])
# share a clkbuf sub-tree instead of being split across clkbuf_leaf_1
# vs clkbuf_leaf_2.
