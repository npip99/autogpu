...
smem has 16 banks (NUM_BANKS=16) each instantiating one sram_1rw, which on
asap7 wraps a fakeram7_256x32 macro (8.36 x 42 um). Lay them out in an
8-col x 2-row grid with wide horizontal channels for the rd_out / wd_in
...
def main() -> None:
    ...
    tcl = ["# Auto-generated by tech/asap7/orfs/scripts/gen_smem_floorplan.py",
           "# 16 fakeram7_256x32 macros in an 8x2 grid.", ""]
    placements = []
    for r in range(ROWS):
        for c in range(COLS):
            b = r * COLS + c
